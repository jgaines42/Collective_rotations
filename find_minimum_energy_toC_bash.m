%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function [using_E] = find_minimum_energy_toC_bash(pdb_names, variant,database, group, min_tostart,folder_data,folder_pdb)
%
% Runs residue rotations to create the data used by combined rotations code
% Returns all states of each amino acid in the group that have an energy
% <= using_E. Clashes are checked between moving residues and all
% non-moving residues in the protein (but not between moving residues)
%
% Input:
%   pdb_names: 3 letter code of the PDB
%   variant: which bond length and angle variant
%   group: which cluster of residues
%   min_tostart: index of which loop of energy is being run. If 0, initial
%       energy to look for is 0.0001, if not, last run is loaded and energy
%       is set to 2x last value
%   folder_data: folder to load and save data to
%   folder_pdb: folder containing the pdb.mat file
%
% Output:
%   Creates *.txt files for use in the recursive algorithm.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [using_E] = find_minimum_energy_toC_bash(pdb_names, variant, group, min_tostart,folder_data,folder_pdb)


if min_tostart > 1
    f = fopen(strcat(folder_data, pdb_names, '_loc_out_', num2str(variant),'_g', num2str(group), '.txt'));
    using_E = fscanf(f,'%f');
    fclose(f);
    using_E = using_E*2;
    min_tostart = using_E;
...
% Load the PDB file
PDB_name = strcat(folder_pdb, pdb_names, '.mat');
load(PDB_name);
...
%Find the residues to be rotated
load(strcat(folder_pdb, pdb_names, '_all_core_binned_2.mat'));
...
for i = 1:size(all_moving,1)
    ...
    if  ismember(resiName, {'Leu','Ile', 'Phe', 'His','Tyr' ,'Trp'})
        all_DOF(i) = 2;
    elseif resiName == 'Met'
        all_DOF(i) = 3;
    else
...
if size(Residue_id,2)>1 && size(Residue_id,2) < 11
    ...
    while not_found
        ...
        while residues <= size(Residue_id,2) && continue_res;
            ...
            [all_coordinates, dihedrals] = set_up_recursion_SingleRes_rot(Residue_id(residues), Residue_names{residues}, tempModel2, all_moving, min_tostart);
            if size(dihedrals,1) > 0
                save(strcat(folder_data, pdb_names, '_', Residue_names{residues}, num2str(Residue_id(residues)), '_dihedrals_fast2_', num2str(variant), '.mat'), 'dihedrals');
                switch residues
                    case 1
        ...
        % The next several steps attempt to cut down on the list that will
        % be searched by finding a local minimum
        if continue_res
            min_loc = zeros(1,size(Residue_id,2));
            min_energy = min_tostart;
            max_depth = size(Residue_id,2);
            
            % See what the energy is of the combo of the lowest energy state
            % of each residue
            if max_depth <= 10
                min_energy_1 = set_energy_cutoff_2(coor_m1,coor_m2,coor_m3,coor_m4,coor_m5,coor_m6,coor_m7, coor_m8, coor_m9, coor_m10,max_depth);
                
                %If this configuration has lower energy than the original
                %energy cutoff, remove all configurations with higher
                %energy (since goal is to find global minimum)
                if min_energy_1 < min_energy
                    ...
                    for resi = 1:size(Residue_id,2)
                        ...
                        load(strcat(folder_data, pdb_names, '_', Residue_names{resi}, num2str(Residue_id(resi)), '_dihedrals_fast2_', num2str(variant), '.mat'));
                        ...
                        save(strcat(folder_data, pdb_names, '_', Residue_names{resi}, num2str(Residue_id(resi)), '_dihedrals_fast2_', num2str(variant), '.mat'), 'dihedrals');
            ...
            if max_depth > 2
                ...
                %Do random sampling to find a lower state
                ...
                % If nothing can be found with energy less than or equal to
                % cutoff, then not_found gets triggered and stuff stops
                % running
                run_pairwise;
                %%%%%%%%%%%%%%%%%%
                here = 1;
            ...
            %Write to file so you can run c++ code
            
            if not_found == 0
                for to_write = 1:max_depth
                    f = fopen(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_Xcoordinates_', num2str(variant), '.txt'), 'w');
                    g = fopen(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_Ycoordinates_', num2str(variant), '.txt'), 'w');
                    h = fopen(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_Zcoordinates_', num2str(variant), '.txt'), 'w');
                    ...
                    f = fopen(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_energy_', num2str(variant), '.txt'), 'w');
                    ...
                    f = fopen(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_atomSizes_', num2str(variant), '.txt'), 'w');
                    ...
                    % Save grouping info of dihedrals
                    load(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_dihedrals_fast2_', num2str(variant), '.mat'));
                    ...
                    f = fopen(strcat(folder_data, pdb_names, '_', Residue_names{to_write}, num2str(Residue_id(to_write)), '_dihedral_groups_', num2str(variant), '.txt'), 'w');
                ...
                f = fopen(strcat(folder_data, pdb_names, '_parameters_', num2str(variant),'_g', num2str(group), '.txt'), 'w');
                if min_energy == 0
