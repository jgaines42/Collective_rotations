% make initial mutation
% Change lines 5-9, 14, line 14 is the title of the pdb file to read
% The pdb will be saved as a .mat file as specified in line 9
function [Position, rest_of_pro] =get_variant(allDipeptide, resiName, rest_of_pro, variant)

Position = zeros(size(allDipeptide,1), 3);
residues = 1;
...
% above line loads Resi_coordinates
...
%  InitChi2=calcDA2(iChi2Array,TempPosition);
...
total_found =100;
rng(variant);
x = randi(size(Resi_coordinates,2)/3,100);
i = x(variant);
for multi_pdb = 1%Make 100 dipeptides with new bl/ba
    
    %Cycle over coordinates from the database until you find one that
    %has a similar Phi/Psi and use that
